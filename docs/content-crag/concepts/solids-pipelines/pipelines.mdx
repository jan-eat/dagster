...
| <PyObject module="dagster" object="GraphDefinition" /> | A graph definition, which is a set of ops (or [sub-graphs](/concepts/solids-pipelines/composite-solids)) wired together. Forms the core of a job. Typically constructed using the <PyObject object="job" decorator /> decorator. |
...
You can define [schedules](/concepts/partitions-schedules-sensors/schedules) to execute jobs at fixed intervals or define [sensors](/concepts/partitions-schedules-sensors/sensors) to trigger them jobs when external changes occur.
...
When a defining job, you can provide [resources](/concepts/resources), [configuration](/concepts/configuration/config-schema), [hooks](/concepts/solids-pipelines/solid-hooks), tags, and an [executor](/deployment/executors) (follow the links for explanation of how to use each of these).
...
`to_job` accepts the same arguments as the <PyObject object="job" decorator /> decorator: you can provide [resources](/concepts/resources), [configuration](/concepts/configuration/config-schema), [hooks](/concepts/solids-pipelines/solid-hooks), tags, and an [executor](/deployment/executors).
...
You can supply a config dictionary. The supplied dictionary will be used to configure the job whenever the job is launched. It will show up in the Dagit Playground and can be overridden.

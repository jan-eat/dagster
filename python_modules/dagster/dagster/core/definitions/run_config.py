...
from .solid import NodeDefinition, SolidDefinition, CompositeSolidDefinition
...
def solid_config_field(
    ...
    field_aliases = {"graph": "solids"} if is_using_graph_job_op_apis else {"solids": "ops"}
...
def composite_solid_config_field(node, resource_defs, handle, ignored, is_using_graph_job_op_apis):
    composite_solid_def = node.definition.ensure_graph_def()
    fields = {
        "inputs": get_inputs_field(
            node,
            composite_solid_def.dependency_structure,
            resource_defs,
            ignored,
        ),
        "outputs": get_outputs_field(node, resource_defs),
...
def graph_config_field(node, resource_defs, handle, is_using_graph_job_op_apis):
    graph_def = node.definition.ensure_graph_def()
    return Field(
        define_node_dictionary_cls(
